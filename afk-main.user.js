...
// @author       afk-plugin
// @license      MIT
// @homepageURL  https://github.com/qcc781192000/idle-lineage-afk-plugin
// @supportURL   https://github.com/qcc781192000/idle-lineage-afk-plugin/issues
// @updateURL    https://raw.githubusercontent.com/qcc781192000/idle-lineage-afk-plugin/main/afk-main.user.js
// @downloadURL  https://raw.githubusercontent.com/qcc781192000/idle-lineage-afk-plugin/main/afk-main.user.js
...
(function () {
    ...
    function initSquadLayoutModule() {
        ...
        function tabCount(type, list) {
            var down = list.filter(function (x) { return !!x._downed; }).length;
            return '<button type="button" data-afk-squad-tab="' + type + '" class="' + (activeType === type ? 'active' : '') + '"><span>' + icons[type] + ' ' + labels[type] + '</span><b>' + list.length + '</b>' + (down ? '<em>' + down + '倒地</em>' : '') + '</button>';
        }
        function rebuildTeam(data) {
            ...
            root.innerHTML = '<div class="afk-squad-typebar">' + tabCount('mercenary', data.mercenary) + tabCount('pet', data.pet) + tabCount('summon', data.summon) + '</div><div class="afk-squad-type-summary"><span>' + icons[activeType] + ' ' + labels[activeType] + '</span><small>共 ' + list.length + '・出戰 ' + active + (down ? '・倒地 ' + down : '') + '</small></div><div class="afk-squad-list">' + content + '</div>';
        }
        ...
        document.addEventListener('click', function (e) {
            ...
            if (reserveToggle) { e.preventDefault(); e.stopPropagation(); reserveOpen = !reserveOpen; localStorage.setItem(PET_GROUP_KEY, String(reserveOpen)); lastTeamSig = ''; queue(); return; }
